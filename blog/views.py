...
def index(request):
    ...
    most_popular_posts = (
        Post.objects.annotate(likes_count=Count("likes")).order_by("-likes_count").prefetch_related("author")[:5]
    )
    ...
    most_fresh_posts = (
        Post.objects.order_by("-published_at").annotate(comments_count=Count("comments")).prefetch_related("author")[:5]
    )
